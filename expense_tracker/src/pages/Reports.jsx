...
ChartJS.register(
  ...
  Tooltip,
  Legend,
   ChartDataLabels // { changed code } register datalabels plugin so labels show without hover
);
...
const Reports = () => {
  ...
  filteredTransactions.forEach(transaction => {
    ...
    if (!groupedData[date]) {
      groupedData[date] = { income: 0, expense: 0, selfTransfer: 0 };
    }

    const type = (transaction.transType || '').toString().trim().toLowerCase();
    const amount = parseFloat(transaction.amount) || 0;
    const toTag = (transaction.tag || '').toString().trim().toLowerCase();

    // Classify transaction into buckets
    if (type === 'income') {
      ...
      // Special case: if destination/to account is "other" or "others", treat as expense
      if (toTag === 'other' || toTag === 'others') {
        groupedData[date].expense += amount;
      } else {
        groupedData[date].selfTransfer += amount;
      }
    } else {
      // fallback: if unknown type, treat as expense to be conservative
      groupedData[date].expense += amount;
    }
  });
  ...
  const selfTransferData = dates.map(date => groupedData[date].selfTransfer);
    ...
    datasets: [
      {
        ...
        tension: 0.1,
        fill: false,
        pointRadius: 6,          // { changed code } larger visible points
        pointHoverRadius: 8,
        datalabels: { color: '#095' } // optional per-dataset color
      },
      {
        label: 'Expense',
        data: expenseData,
    ...
    <div style={{ display: 'flex', justifyContent: 'center', minHeight: '100vh', margin: 0, padding: 0 }}>
      <div style={{ width: '100%', margin: 0, padding: 0 }}>
        ...
        {showModal && (
         ...
         <div style={{ ...styles.chartContainer, height: 520 /* { changed code } keep large container but set explicit height */ }}>
        <Line 
          data={chartData}
          options={{
            responsive: true,
            ...
            plugins: {
              ...
              datalabels: {                       // { changed code } show labels always
                display: true,
                anchor: 'end',
                align: 'top',
                formatter: (value) => {
                  // hide zero labels for clarity
            ...
            elements: {
              point: {
                radius: 6 // fallback for all points
              }
            },
            scales: {
              x: {
